...
if [ $PARAMS -lt "$expected_args" ]; then
  	echo "Usage: autoDeploy.sh, use -o if you want to proceed if the port is already in use || configure parameters GITHUB_URL BRANCH CONTAINER_TAG LOCAL_PORT CONTAINER_PORT SSH_ADDRESS SSH_CREDENTIALS [ENVIRONMENT_VARS] in autoDeployParams.conf"
  	exit 1
else
